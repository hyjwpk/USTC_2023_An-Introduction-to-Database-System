...
王琛 PB20111696                贡献占比：50%              前后端框架实现、界面ui设计与对应实验报告撰写

王骥扬 PB20111697            贡献占比：50%              需求分析、数据库业务逻辑实现与对应实验报告撰写
...
​		与上面的关系设计进行对比，该数据模式的关系中每一分量不可再分，满足1NF。每一个非主属性完全函数依赖于码，满足2NF。每一个非主属性不传递依赖与码，满足3NF的要求。综上，该数据库模式满足3NF的设计要求。
