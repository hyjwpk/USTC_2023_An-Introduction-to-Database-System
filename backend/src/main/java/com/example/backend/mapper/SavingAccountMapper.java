...
public interface SavingAccountMapper {
    ...
    Integer saving(@Param("SavingInteract") SavingInteract savingInteract, Map<String,Object> map);

    @Options(statementType = StatementType.CALLABLE)
    @Select("Call saving_interact (2, #{SavingInteract.account_id}, #{SavingInteract.money}, #{map.status, mode=OUT, jdbcType=INTEGER});")
    Integer withdraw(@Param("SavingInteract") SavingInteract savingInteract, Map<String,Object> map);
